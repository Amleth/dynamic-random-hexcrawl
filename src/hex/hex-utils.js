...
import {
  autres_communautes,
  cite,
  communautes_feeriques,
  draw,
  getValue,
  natural_obstacles as natural_obstacles,
  relief as reliefs,
  vegetation as vegetations
} from '../data/oltree';
...
export const initHexes = (mapRadius) => {
  ...
  const available_coordinates = lodash.shuffle(
    hexes
      .filter(_ => distanceFromCenter(_.q, _.r) < mapRadius)
      .map(_ => ({q: _.q, r: _.r}))
      .toArray()
  );

  const communities = [draw(cite)]
    .concat([...getValue(draw(autres_communautes))])
    .concat([...getValue(draw(communautes_feeriques))]);

  if (communities.length <= available_coordinates.length) {
    communities.map(community => {
      const pos = available_coordinates.pop();
      hexes = updateHex(mapRadius, hexes, pos.q, pos.r, {
        community: community,
        communityPosition: chance.integer({min: 0, max: 6})
      });
    });
  }

  // Make an island
  hexes = setOuterRimRelief(mapRadius, hexes, 0);
...
export const position = _ => {
  ...
  throw new Error('A hex position must be 0, 1, 2, 3, 4, 5 or 6.')
};